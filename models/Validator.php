...
class Validator {
    ...
    function validate($data, $rules) {
        ...
        return $valid;
           ...
           function checkNames ($value,$key){
            //   echo $value;
       for ($i=0;$i<strlen($value);$i++)
       {   
           if (ord ($value[$i]) == '34' ||  ord ($value[$i]) == '36' ||ord ($value[$i]) == '39' 
               || ord ($value[$i]) == '290' || ord ($value[$i]) == '292' || ord ($value[$i]) == '295' )
            throw new Exception("Error: the $key is not valid");
